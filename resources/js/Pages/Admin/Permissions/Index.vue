<template>
    <app-layout-admin title="Permisos">
        <template #header>
                                ...
                                <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                                        <div class="flex items-center justify-end p-5 w-full bg-white">
                                            <Link :href="route('admin.permissionCreate')" class="inline-flex items-center px-4 py-2 bg-dark-brand border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-25 transition">
                                                Crear Permiso
                                            </Link>
                                        </div>
                                        <table class="min-w-full divide-y divide-gray-200">
                                            ...
                                            <tbody class="bg-white divide-y divide-gray-200" v-if="permissions.data[0]">
                                                <tr v-for="permission in permissions.data" :key="permission.id" class="odd:bg-white even:bg-gray-50">
                                                    ...
                                                    <td class="text-center px-6 py-4 whitespace-nowrap text-sm font-medium flex justify-center items-center">
                                                        <Link :href="route('admin.permissionEdit', permission.id)" class="inline-flex items-center px-4 py-2 mr-5 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest bg-orange-500 hover:bg-orange-700 active:bg-orange-900 focus:outline-none focus:border-orange-900 focus:ring focus:ring-orange-300 disabled:opacity-25 transition" >
                                                            Editar
                                                        </Link>
                                                        <DangerButton @click="deletePermission(permission.id)" :key="permission.id">
                                                            Eliminar
                                                        </DangerButton>
                                                        <ConfirmationModal :show="show(permission.id)" @close="deletePermission(permission.id)" :key="permission.id">
                                                            <template #title>
                                                                Eliminar Permiso
                                                            </template>
                                                            <template #content>
                                                            ...
                                                            <template #footer>
                                                                <form @submit.prevent="submit(permission.id)" class="mr-4">
                                                                    <DangerButton :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                                                                        Eliminar
                                                                    </DangerButton>
                                                                </form>
                                                                <jet-secondary-button @click="deletePermission()">
                                                                    Cerrar
                                        ...
                                        <pagination v-if="permissions.data[0]" :paginating="permissions"/>
