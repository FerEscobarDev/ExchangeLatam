<template>
    <app-layout-admin title="Detalle del retiro">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-700 leading-tight">
                <Link :href="route('dashboard')" class="text-blue-brand-gradient1 font-semibold hover:text-blue-brand">Dashboard/</Link><Link :href="route('withdrawal.index')" class="text-blue-brand-gradient1 font-semibold hover:text-blue-brand">Retiros de FBS/</Link>Vista Detallada
            </h2>
        </template>
                ...
                <div class="container mx-auto sm:px-6 lg:px-8">
                    <div class="bg-white shadow-xl sm:rounded-lg flex flex-col">
                        <div class="grid grid-cols-6 p-4 mx-auto w-full xl:w-4/5  mt-5 h-min">
                            <div :class="{'md:col-span-6': !withdrawal.comment}" class="col-span-6 md:col-span-3 flex flex-col justify-between items-center">
                                ...
                                <div class="rounded-lg text-dark-brand w-80 flex flex-col items-start">
                                    <span class="flex justify-between items-center w-full">
                                        <span class="text-sm text-gray-600 mr-2">Cliente: </span><span class="text-md font-semibold text-dark-brand">{{withdrawal.user.name + ' ' + withdrawal.user.lastname}}</span>
                                    </span>
                                    <span class="flex justify-between items-center w-full">
                                        <span class="text-sm text-gray-600 mr-2">Broker: </span><span :class="{'text-green-500' :withdrawal.transactionable.broker.id === 1, 'text-blue-800' :withdrawal.transactionable.broker.id === 2}" class="text-sm font-semibold">{{withdrawal.transactionable.broker.name}}</span>
                                    </span>
                                    <span class="flex justify-between items-center w-full">
                                        <span class="text-sm text-gray-600 mr-2">Cuenta de trading: </span><span class="text-sm font-semibold text-dark-brand">{{withdrawal.transactionable.number}}</span>
                                    </span>
                                    <span class="flex justify-between items-center w-full">
                                        <span class="text-sm text-gray-600 mr-2">Fecha de solicitud: </span><span class="text-sm font-semibold text-dark-brand">{{dateFormat(withdrawal.application_date)}}</span>
                                    </span>                                  
                        ...
                        <div class="grid grid-cols-6 p-4 mx-auto w-full xl:w-4/5 h-min">
                            <div class="col-span-6 md:col-span-3 flex flex-col justify-between items-center">
                                ...
                                <div v-else class="rounded-lg bg-slate-200 text-dark-brand p-4 w-80 flex flex-col items-start">
                                    ...
                                    <span class="font-semibold">{{withdrawal.user.name + ' ' + withdrawal.user.lastname}}</span>
                                    <span class="flex">
                                        <span v-if="withdrawal.user.data_user.doc_type === 1" class="text-sm text-gray-600 mr-1">Cédula: </span>
                                        <span v-else-if="withdrawal.user.data_user.doc_type === 2" class="text-sm text-gray-600 mr-1">Cédula de extranjería: </span>
                                        <span v-else="withdrawal.user.data_user.doc_type === 3" class="text-sm text-gray-600 mr-1">Pasaporte: </span>
                                        <span class="text-sm font-semibold text-dark-brand">{{withdrawal.user.data_user.doc_num}}</span> 
                                    </span>
                                    <span class="flex">
                            ...
                            <div class="col-span-6 md:col-span-3 flex flex-col justify-between items-center mt-5 md:mt-0">
                                ...
                                <div class="rounded-lg text-dark-brand w-80 flex flex-col items-center">
                                    ...
                                    <span class="flex justify-between items-center w-full">
                                        <span class="text-sm text-gray-600 mr-1">Descuento VIP: </span><span class="font-semibold text-dark-brand">$ {{withdrawal.rebate.toLocaleString()}}</span>
                                    </span> 
                                    <div class="mx-auto w-4/5 border-0 h-[1px] bg-gradient-to-r from-slate-300 via-blue-brand to-slate-300 my-5"></div>
                                    <span class="flex justify-between items-center w-full">
                                        <span class="text-base text-dark-brand mr-1">Total a pagar COP: </span><span class="font-semibold text-dark-brand">$ {{withdrawal.total.toLocaleString()}}</span>
                                    </span>
                                </div>   
                            </div>
                        </div>
